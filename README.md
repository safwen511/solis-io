...
An authentic embedded ELF enables the Cilium source to attempt the count-only load and attach; without it, preflight returns `availability.available: false` and `availability.status: object_unavailable`. It never pretends attachment succeeded. `experimental_not_implemented` is reserved for the still-missing request-pointer latency and VM-attribution implementation. Automation must inspect `availability.available` and `availability.status`, not process exit status alone. Object loading and attachment distinguish `object_invalid`, `object_load_failed`, `btf_incompatible`, `btf_missing`, `typed_tracepoint_missing`, `unsupported_endianness`, `permission_denied`, `verifier_rejected`, `attach_failed`, and `cleanup_failed`. Failed JSON includes the exact bounded stage, effective UID, underlying diagnostic, effective capability mask and relevant named capability bits when readable, lockdown mode, memlock limits, `perf_event_paranoid`, and `unprivileged_bpf_disabled`. Cleanup failures are retained as structured `ebpf_cleanup` unavailable sections even when another collection stage also fails. Solis never invokes `sudo` internally.

Developer scaffolding includes symbolic typed-BTF capability requirements, a lifecycle-managed kernel-source interface with structured permission/unsupported/verifier failures, a Cilium count-only loader, and a bounded userspace aggregator. The aggregator keeps fixed buckets (`<100 us` through `1 s+`) per VM, device, and operation instead of retaining individual request latencies. Min, max, count, and average use exact sanitized event values; p50, p95, and p99 are explicitly approximate bucket-derived estimates. Fake event and loader sources are exercised only in tests. A generated object enables real count-only attachment, but request-pointer latency and VM ownership attribution remain unimplemented.
...
- Working eBPF block latency is attributed to the host or shared storage path, not to an individual VM. The experimental `vm-block-latency` model, mapper, parsers, capability/source contracts, bounded aggregation, typed-BTF count-only loader, and an authentic generated count-only object exist, but request-pointer latency plus VM ownership attribution are not implemented yet.
