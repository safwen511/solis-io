...
- Expose an experimental VM block-latency JSON contract, libvirt cgroup-inode mapper, validation parsers, and a Cilium typed-BTF host request-latency collector. Request pointers remain bounded kernel-only correlation keys; CO-RE request metadata classifies host operations and devices, while VM ownership attribution remains deliberately unimplemented.
...
`ebpf vm-block-latency` maps validated libvirt QEMU cgroup v2 inode IDs to experimental block-request latency aggregates without reading command lines or environments. Its kernel ownership path is:
...
The collector correlates `block_rq_issue` and `block_rq_complete` by an opaque request identity inside a bounded kernel hash map; that identity is never emitted. On issue, verifier-safe CO-RE reads extract `request.cmd_flags`, `request.part`, `block_device.bd_dev`, `request.bio`, `bio.bi_blkg`, `blkcg_gq.blkcg`, `blkcg.css.cgroup`, `cgroup.kn`, and `kernfs_node.id`. Completion updates fixed per-CPU host, device/operation, and cgroup/device/operation histograms. Userspace reads only sanitized aggregates and attributes a row only when its cgroup ID exactly matches one of the validated libvirt VM cgroup IDs.

Successful VM-attributed output is labeled `collection_mode: typed_btf_vm_attributed_latency` and `attribution_method: blkcg_cgroup_id_to_libvirt_vm`. `attribution_summary` records attributed and unattributed operations, attributed percentage, and matched VM count. Quality is `available` at no more than 5% unattributed work, `degraded` above 5% through 25%, and `unavailable` above 25%, when no VM matches, or when no completed operation is attributed. Host totals retain all completed request-latency events; VM totals contain exact cgroup-ID matches only. Missing bio/blkcg ownership, unknown cgroups, lookup misses, duplicate issues, map pressure, and incomplete requests remain explicit rather than being assigned to a VM.

Each report includes `vm_attribution_preflight`, which checks that ownership path in kernel BTF. A successful runtime collection changes its status from `preflight_only` to `enabled`; a failed or unsupported run retains the preflight status and missing fields. `ebpf doctor` reports request metadata, ownership-field preflight, and runtime readiness separately. Doctor remains non-invasive: it never loads or attaches a program, so runtime readiness is explicitly “not attempted.”

An authentic embedded ELF matching the C source enables the Cilium source to attempt the experimental load and attach; without it, preflight returns `availability.available: false` and `availability.status: object_unavailable`. A stale object fails loading rather than fabricating results. Automation must inspect `availability.available`, `availability.status`, `attribution_quality`, and the explicit unattributed counters, not process exit status alone. Object loading and attachment distinguish `object_invalid`, `object_load_failed`, `btf_incompatible`, `btf_missing`, `typed_tracepoint_missing`, `unsupported_endianness`, `permission_denied`, `verifier_rejected`, `attach_failed`, and `cleanup_failed`. Failed JSON includes the exact bounded stage, effective UID, underlying diagnostic, effective capability mask and relevant named capability bits when readable, lockdown mode, memlock limits, `perf_event_paranoid`, and `unprivileged_bpf_disabled`. Cleanup failures are retained as structured `ebpf_cleanup` unavailable sections even when another collection stage also fails. Solis never invokes `sudo` internally.

The kernel request map is capped at 65,536 entries, and device and cgroup/device aggregate maps are each capped at 4,096 entries. Duplicate issues, completion lookup misses, map-update failures, missing ownership, unknown cgroups, and requests incomplete at window end are reported explicitly. Latency uses 14 fixed buckets (`<100 us` through `1 s+`); p50, p95, and p99 are approximate bucket-derived estimates, while min, max, count, and average use exact aggregate values. No ring buffer or unbounded request-latency slice is used. Fake event and loader sources are exercised only in tests.

The loader validates the serialized Go key sizes against the embedded ELF and loaded map metadata before iteration. An ABI mismatch is reported as `object_invalid` during object validation or `map_layout_mismatch` during map reading, with the map name and both key sizes; it is never treated as an empty attribution result.

For `tp_btf`, kernel BTF describes each trace callback typedef with a leading internal `void *` callback-data parameter. That parameter is not part of the effective BPF program context. The typed-BTF programs therefore declare `block_rq_issue(struct request *)` and `block_rq_complete(struct request *, blk_status_t, unsigned int)`. Preflight and `ebpf doctor` report both the full kernel typedef parameters and the effective program parameters so a kernel prototype mismatch fails clearly before load.
...
Use `--all-vms` to make the all-running-VM context explicit, or select one or two exact inventory VM names with `--victim` and `--suspect`; these selectors do not imply VM attribution. `--device` remains rejected with `device_filter_unsupported`: device metadata is report-only until selector-to-`major:minor` resolution and filter semantics are validated. `--output` writes the same JSON document to a mode-`0600` temporary file and atomically renames it while retaining JSON on stdout. The parent directory must already exist. Symbolic-link and non-regular targets are rejected, and Linux parent directories are opened through no-follow traversal before writing.
...
- Experimental `vm-block-latency` can produce blkcg/cgroup-ID-correlated per-VM histograms on a compatible privileged host, but the attribution is not exact or production-hardened. Request merging, requeues, missing bio/blkcg ownership, cgroup mapping changes, stacked devices, map pressure, and kernel/BTF drift can leave work unattributed or make physical-layer interpretation ambiguous.
...
- Validate experimental request → bio → blkcg ownership and per-VM histograms across supported kernel/libvirt/storage combinations, including restart, requeue, merge, stacked-device, and instrumentation-loss scenarios.
