...
Task 4 correlates `block_rq_issue` and `block_rq_complete` by request address inside a bounded kernel hash map. The address is never emitted. Completion updates fixed per-CPU latency aggregates; userspace reads only sanitized aggregates and instrumentation counters. Tasks 5 and 6 add verifier-safe CO-RE reads of only `request.cmd_flags`, `request.part`, and `block_device.bd_dev`, providing read/write/flush/discard/unknown classification and `major:minor` host device aggregates. The programs still do not read `bio`, `blkcg`, or cgroup ownership and do not attribute activity to a VM. Successful output remains labeled `collection_mode: typed_btf_request_correlation_host_only`, `attribution_method: host_request_correlation_no_vm_attribution`, and `attribution_quality: unavailable`. Per-VM operation and latency totals remain zero.

Each report includes `vm_attribution_preflight`, which checks the kernel BTF ownership path (`request.bio` → `bio.bi_blkg` → `blkcg_gq.blkcg` → `blkcg.css.cgroup` → `cgroup.kn` → `kernfs_node.id`). This is non-invasive field availability only. Even when every field is present, status is `preflight_only`: runtime ownership extraction, cgroup-to-libvirt validation, and VM attribution are not enabled. `ebpf doctor` reports request-metadata support separately from this ownership preflight and always labels VM attribution as `NOT ENABLED / PREFLIGHT ONLY`.
